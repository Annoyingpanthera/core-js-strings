...
function extractNameFromTemplate(value) {
  ...
  const nameSubstring = value.substring(commaIndex + 2, exclamationIndex);
  const trimmedName = nameSubstring.trim();
  return trimmedName;
}
...
function unbracketTag(str) {
  ...
  const exclamationIndex = str.lastIndexOf('>');
  const tagSubstring = str.substring(commaIndex + 1, exclamationIndex);
  return tagSubstring;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-zA-Z]/g, function (char) {
    ...
    return String.fromCharCode(
      ((charCode - startCode + offset) % 26) + startCode
    );
  });
}
